...
import { Card, CardBody, CardHeader, Button, Avatar, Chip, Divider, Progress, RadioGroup, Listbox, ListboxItem } from "@nextui-org/react";
...
export default function CheckoutForm({ user }: { user: User | null }) {
  ...
  const getOrder = useCallback(async () => {
    ...
    try {
      ...
      if (order?.payment_status !== "PENDING") throw new Error("Order not pending !");
    ...
    return (
      <Card className="mx-auto mt-10 max-w-sm">
        <div className="flex items-center space-x-2 p-5">
          <Icon icon="mdi:alert-circle" className="text-danger text-4xl" />
              ...
              <ListboxItem
                ...
                endContent={
                  <div className="flex flex-none">
                    {item.isCompleted || checkoutSteps.indexOf(item) <= currentStep ? (
                      <Icon className="text-success" icon="solar:check-circle-bold" width={30} />
                    ) : (
                      <Icon
          ...
          <div className="space-y-4">
            {orderItems.map((item: any) => (
              <div key={item.id} className="flex justify-between items-center">
                <span>{item.product.name} <Chip size="sm" variant="flat">x{item.quantity}</Chip></span>
                <span className="font-semibold">{(item.quantity * item.price_at_time).toFixed(2)} €</span>
              </div>
            ))}
            <Divider />
